page 50122 "E3 HIS Collection Stagging"
{

    ApplicationArea = All;
    Caption = 'Collection Stagging';
    PageType = List;
    Editable = true;

    SourceTableView = Sorting("Entry No.") where("General Entries Created" = filter(false), "HIS Bill Type" = filter('Collection'));
    SourceTable = "E3 HIS Collection Staging";
    UsageCategory = Lists;
    ...
    actions
    {
        area(Processing)
        {
            action("Create Collection Entries")
            {
                ApplicationArea = All;
                ...
                begin
                    HISIntegration.InitGenJnlLineCollectionStaging();

                end;
            }
            action("Post Collection Entry")
            {
                ApplicationArea = All;
                Image = PostBatch;
                Promoted = true;
