page 50169 "Create Bank Payment"
{
        ...
        area(content)
        {
            group(Control6)
            {
                ...
                field("Batch Name"; JournalBatchName)
                {
                    ...
                    Caption = 'Batch Name';
                    ShowMandatory = true;
                    TableRelation = "Gen. Journal Batch".Name where("Template Type" = const(Payments), Recurring = const(false));
                    ...
                    trigger OnValidate()
                    begin
                        if JournalBatchName <> '' then BatchSelection(JournalTemplateName, JournalBatchName, false);
                    end;
                }
                ...
                field("Bank Account"; BalAccountNo)
                {
                    ...
                    begin
                        ...
                        if not BankAccount.Get(BalAccountNo) then
                            Error('Selected Bank Account %1 does not exist.', BalAccountNo);
                    end;
                }
    ...
    var
        ...
        BankIntegration: Codeunit "E3 Bank Integration";
    begin
        PostingDate := WorkDate();
        if not GenJournalTemplate.Get(JournalTemplateName) then Clear(JournalTemplateName);
        if not GenJournalBatch.Get(JournalTemplateName, JournalBatchName) then Clear(JournalBatchName);
        if JournalTemplateName = '' then if BankIntegration.TemplateSelectionSimple(GenJournalTemplate, GenJournalTemplate.Type::Payments, false) then JournalTemplateName := GenJournalTemplate.Name;
    ...
    begin
        if CloseAction = ACTION::OK then begin
            if JournalBatchName = '' then Error(BatchNumberNotFilledErr);
            if Format(PostingDate) = '' then Error(PostingDateNotFilledErr);
            if NextDocNo = '' then Error(SpecifyStartingDocNumErr);
        end;
    end;
    ...
    begin
        ...
        VendorLedgerEntry.SetCurrentKey("Global Dimension 1 Code"); //Sandeep Sir
        VendorLedgerEntry.SetFilter("Amount to Pay", '<>%1', 0);//ak
        ...
        if VendorLedgerEntry.Find('-') then
            repeat
                ...
                if VendorLedgerEntry."Applies-to ID" = '' then begin
                    //VendorLedgerEntry.CalcFields("Amount to Pay");
                    ...
                    TempVendorPaymentBuffer."Global Dimension 1 Code" := VendorLedgerEntry."Global Dimension 1 Code"; //Sandeep Sir
                    ...
                    if CheckCalcPmtDiscGenJnlVend(VendorLedgerEntry."Amount to Pay", VendorLedgerEntry, 0, false) then
                        PaymentAmt := -(VendorLedgerEntry."Amount to Pay" - VendorLedgerEntry.GetRemainingPmtDiscPossible(PostingDate))
                    else
                        PaymentAmt := -VendorLedgerEntry."Amount to Pay";

                    TempVendorPaymentBuffer.Reset();
                    TempVendorPaymentBuffer.SetRange("Vendor No.", VendorLedgerEntry."Vendor No.");
                    TempVendorPaymentBuffer.SetRange("Global Dimension 1 Code", VendorLedgerEntry."Global Dimension 1 Code"); //Sandeep Sir
                    ...
                    VendorLedgerEntry."Amount to Apply" := VendorLedgerEntry."Amount to Pay";
    ...
    begin
        ...
        TempVendorPaymentBuffer.SetFilter("Vendor Ledg. Entry Doc. Type", '<>%1&<>%2', TempVendorPaymentBuffer."Vendor Ledg. Entry Doc. Type"::Refund, TempVendorPaymentBuffer."Vendor Ledg. Entry Doc. Type"::Payment);
        if TempVendorPaymentBuffer.Find('-') then
            repeat
                ...
                else if TempVendorPaymentBuffer.Amount > 0 then
                    GenJnlLine."Document Type" := GenJnlLine."Document Type"::Payment
                else
                ...
                if Vendor."No." <> TempVendorPaymentBuffer."Vendor No." then Vendor.Get(TempVendorPaymentBuffer."Vendor No.");
                ...
                GenJnlLine."Bank Integration" := true;
    ...
    begin
        if GenJnlLine."Dimension Set ID" = 0 then begin
            ...
            if NewDimensionID <> GenJnlLine."Dimension Set ID" then AssignCombinedDimensionSetID(GenJnlLine, DimSetIDArr, NewDimensionID);
        end;
    ...
    begin
        ...
        exit(PaymentToleranceManagement.CheckCalcPmtDisc(NewCVLedgEntryBuf, OldCVLedgEntryBuf2, ApplnRoundingPrecision, false, CheckAmount));
    end;
    ...
    begin
        ...
        if IsHandled then exit(MessageToRecipient);
        ...
        if VendorLedgerEntry."Message to Recipient" <> '' then exit(VendorLedgerEntry."Message to Recipient");
        exit(StrSubstNo(MessageToRecipientMsg, TempVendorPaymentBuffer."Vendor Ledg. Entry Doc. Type", TempVendorPaymentBuffer."Applies-to Ext. Doc. No."));
    end;
    ...
    var
        ...
        BankIntegration: Codeunit "E3 Bank Integration";
    begin
        BankIntegration.CheckTemplateName(CurrentJnlTemplateName, CurrentJnlBatchName);
